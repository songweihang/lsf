...
function SqlOrm.define_model(sql_database, table_name)
    ...
    local orm = require('db.sql.' .. sql_database.options.adapter .. '.orm').new(table_name, quote)
    ...
    function GinModel.create(attrs)
        ...
        return sql_database:execute_and_return_last_id(sql)
    end

    function GinModel.where(attrs, options)
        local sql = orm:where(attrs, options)
        
        local ok,results = sql_database:execute(sql)
        if ok == 200 then
            local models = {}
            for i = 1, #results do
